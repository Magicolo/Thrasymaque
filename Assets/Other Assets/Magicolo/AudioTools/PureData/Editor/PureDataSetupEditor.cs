...
namespace Magicolo.AudioTools {
	...
	public class PureDataSetupEditor : CustomEditorBase {
		...
		void ShowSpatialSetting() {
			...
			setup.spatialSettingsShowing = EditorGUILayout.Foldout(setup.spatialSettingsShowing, "3D Sound Settings");
			if (setup.spatialSettingsShowing && clip != null) {
		...
		void ShowClipSettings() {
			...
			setup.clipInfoShowing = EditorGUILayout.Foldout(setup.clipInfoShowing, "Clip Info");
			if (setup.clipInfoShowing && clip != null) {
				EditorGUI.indentLevel += 1;
				...
				if (info.frequency != info.pureData.generalSettings.SampleRate) {
					EditorGUILayout.HelpBox("This clip's frequency differs from Pure Data's sample rate. This might generate artifacts when playing the clip.", MessageType.Warning);
				}
