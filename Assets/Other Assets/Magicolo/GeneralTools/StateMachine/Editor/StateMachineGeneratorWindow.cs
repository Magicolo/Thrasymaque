...
namespace Magicolo.EditorTools {
	public class StateMachineGeneratorWindow : CustomWindowBase {
		...
		void GenerateLayer() {
			...
			File.WriteAllLines(Application.dataPath.Substring(0, Application.dataPath.Length - 6) + Path.DirectorySeparatorChar + path + Path.DirectorySeparatorChar + layerFileName, layerScript.ToArray());
		...
		void GenerateStates() {
			...
			foreach (string state in states) {
				...
				File.WriteAllLines(Application.dataPath.Substring(0, Application.dataPath.Length - 6) + path + Path.DirectorySeparatorChar + stateFileName, stateScript.ToArray());
			}
